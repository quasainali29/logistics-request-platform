...
export async function addComment(
  ...
  try {
    const [{ data: mentionedProfiles }, { data: authorProfile }, { data: request }] =
      await Promise.all([
    ...
    const link = `${process.env.NEXT_PUBLIC_APP_URL ?? ""}/requests/${requestId}`;
      ...
      (mentionedProfiles ?? [])
        ...
        .map((p) =>
          sendNotificationEmail({
            ...
            subject: `${authorName} mentioned you on ${request?.request_number ?? "a request"}`,
            html: `<p><strong>${escapeHtml(authorName)}</strong> mentioned you in a comment on <strong>${escapeHtml(
              request?.title ?? ""
            )}</strong> (${escapeHtml(request?.request_number ?? "")}):</p><p style="padding:10px 14px;background:#f8fafc;border-radius:6px;color:#0f172a;">${escapeHtml(
              comment
