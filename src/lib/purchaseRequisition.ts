...
const itemColW = [700, 3200, 1000, 1660, 2800];

function imageCell(item: PurchaseRequisitionItem) {
  if (item.imageBuffer) {
    return new TableCell({
...
export async function buildPurchaseRequisitionDocx(
    ...
    rows: [
      new TableRow({
        children: [
          ...
          labelCell("Purchasing category", 1800),
          valueCell(data.purchasingCategory, 2880),
        ],
      }),
      new TableRow({
        children: [
          ...
          labelCell("Vendor", 1800),
          valueCell(data.vendor, 2880),
        ],
      }),
      new TableRow({
        children: [
          ...
          labelCell("Needed by", 1800),
          valueCell(data.neededBy, 2880),
        ],
      }),
      new TableRow({
        children: [
    ...
    rows: [
      new TableRow({
        children: [
          ...
          itemHeaderCell("Purchasing Link", itemColW[4]),
        ],
      }),
      ...data.items.map(
        (item, idx) =>
          new TableRow({
            children: [
              textItemCell(String(item.itemNo ?? idx + 1), itemColW[0]),
              textItemCell(item.description, itemColW[1]),
              textItemCell(String(item.quantity), itemColW[2]),
              imageCell(item),
              textItemCell(item.purchasingLink || "—", itemColW[4]),
            ],
          })
      ),
    ],
    ...
    sections: [
      {
        ...
        children: [
          new Paragraph({
            alignment: AlignmentType.CENTER,
            spacing: { after: 300 },
            children: [new TextRun({ text: "PURCHASE REQUISITION", bold: true, size: 32 })],
          }),
          headerTable,
          new Paragraph({
            spacing: { before: 240, after: 120 },
            children: [new TextRun({ text: "Procurement Details", bold: true, size: 22 })],
          }),
          detailsTable,
          new Paragraph({
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text:
                  "This is to confirm that the above items are required for official use and are approved for procurement, unless otherwise noted above.",
                size: 20,
              }),
            ],
          }),
          new Table({
            ...
            rows: [
              new TableRow({
                children: [sigCell("Procurement Officer"), sigCell("Finance Approval")],
              }),
            ],
          }),
        ],
